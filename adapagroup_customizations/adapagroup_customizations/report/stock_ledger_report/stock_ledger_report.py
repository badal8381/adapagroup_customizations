...
def get_stock_ledger_entries(filters, items):
	...
	if filters.get("batch_no"):
		bundles = get_serial_and_batch_bundles(filters, batches=[filters.batch_no])
	...
	if filters.get('manufacturing_batch_id'):
		batch_no = frappe.get_all("Batch", {"manufacturing_batch_id": ["like", filters.manufacturing_batch_id]}, "name")
		if batch_no:
			batches = [batch.get("name") for batch in batch_no]
			bundles = get_serial_and_batch_bundles(filters, batches=batches)

			if bundles:
				query = query.where(
					(sle.serial_and_batch_bundle.isin(bundles)) | (sle.batch_no.isin(batches))
				)
			else:
				query = query.where(sle.batch_no.isin(batches))
...
def get_serial_and_batch_bundles(filters, batches=None):
